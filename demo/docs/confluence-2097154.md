...
| /api/v1/payments | GET | List payments, optionally filtered by payerId, status, or both | PaymentController#list | APIs, Repository, Testing |
...
| amount | BigDecimal | Minimum 30.00, maximum 30000.00 | Payment amount |
...
- Payment amount must be at least 30.00 and is capped at 30000.00 per transaction by request validation.
...
Validation is implemented through Jakarta Bean Validation annotations in request DTOs and surfaced through GlobalExceptionHandler as HTTP 400 responses. Key constraints: amount minimum 30.00, amount maximum 30000.00, currency must be a 3-letter ISO code.
